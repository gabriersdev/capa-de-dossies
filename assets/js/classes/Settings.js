...
class Settings {
  options = {
    ...
    "armazena-dados": { // Possui a funcionalidade de confirmar armazenar os dados
      "type": "boolean",
      "default": true,
      "propertie": "checked"
    },
  ...
  CRUDoption(action, config_name, config_value) {
    try {
      let settings_saved = JSON.parse(localStorage.getItem("capa-dossie-html-configs")) || "";

      if (isEmpty(settings_saved)) {
        settings_saved = this.createSettingsObject(settings_saved);
      }
      ...
      console.log("Um erro ocorreu ao %s a configuração. Erro: %s", action,);
      try {
